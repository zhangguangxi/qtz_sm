...
@Component(value = "wtCczxIncomeJob")
public class WtCczxIncomeJob {
	/**初始化日志对象*/
	private static LogTool log = LogTool.getInstance(WtCczxIncomeJob.class);
	...
	@Resource(name="wtCczxIncomeSettlementServiceImpl")
	private WtCczxIncomeSettlementService cczxIncomeSettlementService;
	
	@Resource(name="wtWalletServiceImpl")
	private WtWalletService walletService;
	
	public void run(){
		log.debug("仓储中心的收入对账任务开始----------->>>>>>>");
		
		if(lock.tryLock()){
			try {
				long now = System.currentTimeMillis();
				WtCczxIncome cczxIncomeParam = new WtCczxIncome();
				cczxIncomeParam.setUpdateTime(now);
				cczxIncomeParam.setReleaseTime(now);

//				步骤一：将待解冻的收入流水 结算状态设置“结算进行中” 这个中间状态
				int modResult = cczxIncomeService.modIncomeStatToSettling(cczxIncomeParam);
				if(modResult <= 0){
					log.info("仓储中心没有待解冻需要结算的收入流水");
					return ;
				}
				log.info("仓储中心待结算的收入流水条数：" + modResult);
				
//				步骤二： 分组将各个收款人待解冻的收入分组统计
				List<WtCczxIncome> resultList = null;
				resultList = cczxIncomeService.queryTotalAmountGroup(cczxIncomeParam);
				if(!CollectionUtils.isEmpty(resultList)){
					for (WtCczxIncome wtIncome : resultList) {
						//步骤三：开始逐个用户收入结算
						
						WtCczxIncomeSettlement incomeSm = new WtCczxIncomeSettlement();
						try {
							...
							Boolean result = cczxIncomeSettlementService.settlement(incomeSm);
							if(result == false){
							...
							if(e.getErrorType() != ExceptionCode.WALLET_ERROR && e.getErrorType() != ExceptionCode.WALLET_AMOUNT_UNNORMAL){
								...
								while(retryTimes-- > 0){
									try {
										cczxIncomeSettlementService.settlement(incomeSm);	//对账结算重试
										handleResult = true;
										break;	//无异常则 break 循环
									} catch (Exception e1) {
										log.error(e1);
							...
							if(handleResult == false){
								try {
									WtCczxIncome incomeMod = new WtCczxIncome();
									...
									cczxIncomeService.modIncomeStat(incomeMod );
									

									incomeSm.setRemark("自动对账结算失败，errorCode:" + e.getErrorType() + ",errorMessage：" + e.getErrorMessage());
									cczxIncomeSettlementService.addVo(incomeSm);
								} catch (Exception e1) {
									log.error(e1);
								}
							}
