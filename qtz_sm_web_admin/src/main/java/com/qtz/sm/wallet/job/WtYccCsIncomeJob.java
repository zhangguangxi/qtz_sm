...
@Component(value = "wtYccCsIncomeJob")
public class WtYccCsIncomeJob {
	/**初始化日志对象*/
	private static LogTool log = LogTool.getInstance(WtYccCsIncomeJob.class);
	...
	@Resource(name="wtYccCsIncomeSettlementServiceImpl")
	private WtYccCsIncomeSettlementService yccCsIncomeSettlementService;
	
	@Resource(name="wtWalletServiceImpl")
	private WtWalletService walletService;
	
	public void run(){
		log.debug("云仓储管理公司从超市获得的收入对账任务开始----------->>>>>>>");
		
		if(lock.tryLock()){
			try {
				long now = System.currentTimeMillis();
				WtYccCsIncome yccCsIncomeParam = new WtYccCsIncome();
				yccCsIncomeParam.setUpdateTime(now);
				yccCsIncomeParam.setReleaseTime(now);

//				步骤一：将待解冻的收入流水 结算状态设置“结算进行中” 这个中间状态
				int modResult = yccCsIncomeService.modIncomeStatToSettling(yccCsIncomeParam);
				if(modResult <= 0){
					log.info("云仓储管理公司从超市没有待解冻需要结算的收入流水");
					return ;
				}
				log.info("云仓储管理公司从超市待结算的收入流水条数：" + modResult);
				
//				步骤二： 分组将各个收款人待解冻的收入分组统计
				List<WtYccCsIncome> resultList = null;
				resultList = yccCsIncomeService.queryTotalAmountGroup(yccCsIncomeParam);
				if(!CollectionUtils.isEmpty(resultList)){
					for (WtYccCsIncome wtIncome : resultList) {
						//步骤三：开始逐个用户收入结算
						
						WtYccCsIncomeSettlement incomeSm = new WtYccCsIncomeSettlement();
						try {
							...
							Boolean result = yccCsIncomeSettlementService.settlement(incomeSm);
							if(result == false){
							...
							if(e.getErrorType() != ExceptionCode.WALLET_ERROR && e.getErrorType() != ExceptionCode.WALLET_AMOUNT_UNNORMAL){
								...
								while(retryTimes-- > 0){
									try {
										yccCsIncomeSettlementService.settlement(incomeSm);	//对账结算重试
										handleResult = true;
										break;	//无异常则 break 循环
									} catch (Exception e1) {
										log.error(e1);
							...
							if(handleResult == false){
								try {
									WtYccCsIncome incomeMod = new WtYccCsIncome();
									...
									yccCsIncomeService.modIncomeStat(incomeMod );
									

									incomeSm.setRemark("自动对账结算失败，errorCode:" + e.getErrorType() + ",errorMessage：" + e.getErrorMessage());
									yccCsIncomeSettlementService.addVo(incomeSm);
								} catch (Exception e1) {
									log.error(e1);
								}
							}
		...
		log.debug("云仓储管理公司从超市获得的收入对账任务结束------------>>>>>>>>");
	}
