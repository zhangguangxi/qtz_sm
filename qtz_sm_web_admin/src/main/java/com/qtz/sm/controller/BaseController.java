...
//		user.setDmId(123456789);
    ...
    public void exp(HttpServletRequest request, Exception ex,HttpServletResponse response) {
        ...
        if (ex instanceof ServiceException) {
			...
			FieldError fErr = bindResult.getFieldError();
			...
			RespHandler.respArgNotValidEx(new JSONObject(RespMsg.respOperationMsg(code, msg)), response);
		} else {
			RespHandler.respServerError(response);
        }
    }
	 ...
	 * 返回错误信息
	 *
	 * @param errorType    错误信息ID
	 * @param response 响应对象
	 */
	public void getrespError(int errorType,HttpServletResponse response){
		if (errorType == RespCode.not_present_application) {
			RespHandler.respError(RespMsg.NOT_PRESENT_APPLICATION, response);
		}else if (errorType == RespCode.exist_bing_bankcard) {
			RespHandler.respError(RespMsg.EXIST_BING_BANKCARD, response);
		} else if (errorType == RespCode.not_belong_bing_bankcard) {
			RespHandler.respError(RespMsg.NOT_BELONG_BING_BANKCARD, response);
		}else if (errorType == RespCode.not_to_mention_present) {
			RespHandler.respError(RespMsg.NOT_TO_MENTION_PRESENT, response);
		}else if (errorType == RespCode.consent_to_withdrawal) {
			RespHandler.respError(RespMsg.CONSENT_TO_WITHDRAWAL, response);
		}else if (errorType == RespCode.already_to_mention_present) {
			RespHandler.respError(RespMsg.ALREADY_TO_MENTION_PRESENT, response);
		}else if (errorType == RespCode.to_mention_bh_present) {
			RespHandler.respError(RespMsg.TO_MENTION_BH_PRESENT, response);
		}else if (errorType == RespCode.already_to_bh_present) {
			RespHandler.respError(RespMsg.ALREADY_TO_BH_PRESENT, response);
		}else if (errorType == RespCode.to_mention_zh_present) {
			RespHandler.respError(RespMsg.TO_MENTION_ZH_PRESENT, response);
		}else if (errorType == RespCode.wall_not_set) {
			RespHandler.respError(RespMsg.WALL_NOT_SET, response);
		}else if (errorType == RespCode.wall_not_initialized) {
			RespHandler.respError(RespMsg.WALL_NOT_INITIALIZED, response);
		}else {
			RespHandler.respError(RespMsg.bindingBank_error(errorType), response);
		}
	}
}
