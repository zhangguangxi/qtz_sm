...
<%!
    ...
    private HttpURLConnection doHTTPRequest(String uri, byte[] bytes, String method, Map mapHeaderInfo) throws IOException{
        ...
        if(query!=null&&!"".equals(query)){
        	...
        	if(tokenHeader!=null&&!"".equals(tokenHeader)){
        	   String[] token = tokenHeader.split("=");
        		mapHeaderInfo.put("token",token[1]);
        	}
        }
        ...
        if (bytes != null && bytes.length > 0 || method.equals("POST")) {
			...
			String postBody = new String(bytes);
			if(postBody!=null&&!"".equals(postBody)){
				
	        	if(postBody.indexOf("\"token\"")!=-1){
	        		JSONObject jsonObject = JSONObject.fromObject(postBody);
		        	con.setRequestProperty("token",jsonObject.getString("token"));
	        	}
	        }
    ...
    private String getNewTokenIfCredentialsAreSpecified(ServerUrl su, String url) throws IOException{
        ...
        if (isUserLogin || isAppLogin) {
            ...
            if (isAppLogin) {
                ...
                String uri = su.getOAuth2Endpoint() + "tok?client_id=" + URLEncoder.encode(su.getClientId(),"UTF-8") + "&client_secret=" + URLEncoder.encode(su.getClientSecret(), "UTF-8") + "&grant_type=client_credentials&f=json";
                String tokenResponse = webResponseToString(doHTTPRequest(uri, "POST"));
                token = extractToken(tokenResponse, "access_tok");
                ...
                if (url.toLowerCase().contains("/generatetok")){
                    String tokenResponse = webResponseToString(doHTTPRequest(url, "POST"));
                    token = extractToken(tokenResponse, "tok");
                    return token;
                }
                ...
                if (!infoUrl.isEmpty()) {
                    ...
                    if (tokenServiceUri == null || tokenServiceUri.isEmpty()){
                        ...
                        tokenServiceUri = getJsonValue(infoResponse, "tokServicesUrl");
                        ...
                        if (tokenServiceUri.isEmpty()){
                            ...
                            if (!owningSystemUrl.isEmpty()){
                                tokenServiceUri = owningSystemUrl + "/sharing/generateTok";
                            }
                        }
                    ...
                    if (tokenServiceUri != null && !tokenServiceUri.isEmpty()){
                        ...
                        String uri = tokenServiceUri + "?f=json&request=getTok&referer=" + URLEncoder.encode(PROXY_REFERER,"UTF-8") + "&expiration=60&username=" + URLEncoder.encode(su.getUsername(),"UTF-8") + "&password=" + URLEncoder.encode(su.getPassword(), "UTF-8");
                        String tokenResponse = webResponseToString(doHTTPRequest(uri, "POST"));
                        token = extractToken(tokenResponse, "tok");
                    }
                }
    ...
    private String exchangePortalTokenForServerToken(String portalToken, ServerUrl su) throws IOException{
        ...
        String uri = su.getOAuth2Endpoint().substring(0, su.getOAuth2Endpoint().toLowerCase().indexOf("/oauth2/")) +
                "/generateToken?tok=" + URLEncoder.encode(portalToken,"UTF-8") + "&serverURL=" + URLEncoder.encode(url,"UTF-8") + "&f=json";
        String tokenResponse = webResponseToString(doHTTPRequest(uri, "GET"));
        return extractToken(tokenResponse, "tok");
    }

    private String addTokenToUri(String uri, String token) {
        if (token != null && !token.isEmpty())
            uri += uri.contains("?") ? "&tok=" + token : "?tok=" + token;
        return uri;
    }
                        ...
                        if (found) {
                            ...
                            if (serverUrls != null && !serverUrls.isEmpty()) {
                                ...
                                while(m.find()){
                                    ...
                                    String tokenServiceUri = ProxyConfig.getAttributeWithRegex("tokServiceUri", server);
    ...
    public static class ServerUrl {
        ...
        public ServerUrl(String url, String matchAll, String oauth2Endpoint, String username, String password, String clientId, String clientSecret, String rateLimit,
                         String rateLimitPeriod, String tokServiceUri, String hostRedirect){
            ...
            this.tokServiceUri = tokServiceUri;
        ...
        public String getTokenServiceUri(){
            if (this.tokServiceUri == null && tokenServiceMap != null){
                this.tokServiceUri = tokenServiceMap.get(this.url);
            }
            return this.tokServiceUri;
        }

        public void setTokenServiceUri(String value){

            this.tokServiceUri = value;
...
%><%
    ...
    try {
        ...
        boolean hasClientToken = requestUri.contains("?tok=") || requestUri.contains("&tok=") || post.contains("?tok=") || post.contains("&tok=");
        String token = "";
        if (!hasClientToken) {
            // Get new token and append to the request.
            // But first, look up in the application scope, maybe it's already there:
            token = (String)application.getAttribute("tok_for_" + serverUrl.getUrl());
            ...
            if (token != null && !token.isEmpty() && !tokenIsInApplicationScope) {
                ...
                application.setAttribute("tok_for_" + serverUrl.getUrl(), token);
            }
        }
