...
/**
 * <p>Title:WtYccBldglIncomeSettlementServiceImpl</p>
 ...
 */
@Service("wtYccBldglIncomeSettlementServiceImpl")
public class WtYccBldglIncomeSettlementServiceImpl extends BaseServiceImpl<com.qtz.sm.wallet.vo.WtYccBldglIncomeSettlement,java.lang.Long> implements WtYccBldglIncomeSettlementService  {
	/**初始化日志对象*/
	private static LogTool log = LogTool.getInstance(WtYccBldglIncomeSettlementServiceImpl.class);
	/**注入超市收入对账结算记录DAO接口类*/
	@Resource(name="wtYccBldglIncomeSettlementDaoImpl")
    private WtYccBldglIncomeSettlementDao dao;
	...
	*/
	@Override
	protected BizDao<com.qtz.sm.wallet.vo.WtYccBldglIncomeSettlement,java.lang.Long> getDao() {
		return dao;
	}
	/** 
	* 【取得】日志对象
	...
	*/
	...
	@Override
	public boolean settlement(WtYccBldglIncomeSettlement incomeSm) throws ServiceException {
		log.info("云仓储管理公司从便利店管理公司获得的收入结算，请求参数---------->>" + JSONUtils.Object2JSON(incomeSm));
		try {
			...
			WtWallet incomeWt = walletService.getWallet(ownerId, CompanyType.CloudStorage.value(), WalletItemEnum.INCOME.getValue(), CompanyType.StoreManager.value());
			...
			WtWallet incomedWt = walletService.getWallet(ownerId, CompanyType.CloudStorage.value(), WalletItemEnum.INCOMED.getValue(), CompanyType.StoreManager.value());
			if(incomedWt != null){
			...
			WtWallet unpaidWtOfPayer = walletService.getWallet(incomedWt.getPayerId(), CompanyType.StoreManager.value(), WalletItemEnum.UPPAID.getValue(), CompanyType.CloudStorage.value());
			...
			log.info("云仓储管理公司从便利店管理公司获得的收入结算，从对账中钱包中扣减：" + JSONUtils.Object2JSON(incomeWalletMod));
			...
			log.info("便利店管理公司的未付结算，向云仓储管理公司的收款人的未付钱包中增加：" + JSONUtils.Object2JSON(unpaidWtOfPayerMod));
			...
			log.info("云仓储管理公司从便利店公司获得的收入结算，从应收钱包中增加：" + JSONUtils.Object2JSON(incomedWalletMod));
			...
			yccBldglIncomeService.modIncomeStat(wtIncome );
