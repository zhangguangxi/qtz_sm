...
/**
 * <p>Title:WtGylIncomeSettlementServiceImpl</p>
 ...
 */
@Service("wtGylIncomeSettlementServiceImpl")
public class WtGylIncomeSettlementServiceImpl extends BaseServiceImpl<com.qtz.sm.wallet.vo.WtGylIncomeSettlement,java.lang.Long> implements WtGylIncomeSettlementService  {
	/**初始化日志对象*/
	private static LogTool log = LogTool.getInstance(WtGylIncomeSettlementServiceImpl.class);
	/**注入超市收入对账结算记录DAO接口类*/
	@Resource(name="wtGylIncomeSettlementDaoImpl")
    private WtGylIncomeSettlementDao dao;
	...
	*/
	@Override
	protected BizDao<com.qtz.sm.wallet.vo.WtGylIncomeSettlement,java.lang.Long> getDao() {
		return dao;
	}
	/** 
	* 【取得】日志对象
	...
	*/
	...
	@Override
	public boolean settlement(WtGylIncomeSettlement incomeSm) throws ServiceException {
		log.info("供应链收入结算，请求参数---------->>" + JSONUtils.Object2JSON(incomeSm));
		try {
			...
			WtWallet incomeWt = walletService.getWallet(ownerId, CompanyType.SupplyChain.value(), WalletItemEnum.INCOME.getValue(), CompanyType.CloudStorage.value());
			...
			WtWallet incomedWt = walletService.getWallet(ownerId, CompanyType.SupplyChain.value(), WalletItemEnum.INCOMED.getValue(), CompanyType.CloudStorage.value());
			if(incomedWt != null){
			...
			WtWallet unpaidWtOfPayer = walletService.getWallet(incomedWt.getPayerId(), CompanyType.CloudStorage.value(), WalletItemEnum.UPPAID.getValue(), CompanyType.SupplyChain.value());
			...
			log.info("供应链收入结算，从对账中钱包中扣减：" + JSONUtils.Object2JSON(incomeWalletMod));
			...
			log.info("云仓储未付结算，向供应链的收款人的未付钱包中增加：" + JSONUtils.Object2JSON(unpaidWtOfPayerMod));
			...
			log.info("供应链收入结算，从应收钱包中增加：" + JSONUtils.Object2JSON(incomedWalletMod));
			...
			gylIncomeService.modIncomeStat(wtIncome );
