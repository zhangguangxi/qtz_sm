...
/**
 * <p>Title:WtBldglWithdrawalsServiceImpl</p>
 * <p>Description:便利店管理公司提现流水服务实现类</p>
 * <p>Copyright: Copyright (c) 2016</p>
  * <p>Company: 深圳擎天柱信息技术有限公司</p>
 * @author 徐运贤 - xuyunxian@126.com
 * @version v1.0 2016-05-11
 */
@Service("wtBldglWithdrawalsServiceImpl")
public class WtBldglWithdrawalsServiceImpl extends BaseServiceImpl<com.qtz.sm.wallet.vo.WtBldglWithdrawals,java.lang.Long> implements WtBldglWithdrawalsService  {
	/**初始化日志对象*/
	private static LogTool log = LogTool.getInstance(WtBldglWithdrawalsServiceImpl.class);
	/**注入便利店管理公司提现流水DAO接口类*/
	@Resource(name="wtBldglWithdrawalsDaoImpl")
    private WtBldglWithdrawalsDao dao;
	...
	*/
	@Override
	protected BizDao<com.qtz.sm.wallet.vo.WtBldglWithdrawals,java.lang.Long> getDao() {
		return dao;
	}
	/** 
	* 【取得】日志对象
	...
	*/
	...
	public void saveApplyWithdrawals(WtBldglWithdrawals vo) throws ServiceException {
		...
		try {
			...
			//从配置文件中读取便利店管理公司结算周期的设置参数
			String period=BaseProperties.getBaseProperties("bldgl.withdrawals.period");
			...
			wVo.setOwnerType(CompanyType.StoreManager.value());
	...
	public void saveAccepWithdrawals(WtBldglWithdrawals vo) throws ServiceException {
		// TODO Auto-generated method stub
		try {
			WtBldglWithdrawals withdrawals = dao.findVo(vo.getDmId(), null);
			if (null == withdrawals) {
			...
			for (WtWallet w : walletList) {
				if (w.getPayerType() == CompanyType.PPSH.value()) {
	...
	public void saveRecoverWithdrawals(WtBldglWithdrawals vo) throws ServiceException {
		...
		try {
			...
			if (null == wWithdrawals || null == wIncomed) {
				throw new ServiceException(RespCode.wall_not_initialized," 钱包没初始化");
			}
	 ...
	 * @throws DaoException 
	 */
	public void addWtWithdrawalsOperation(WtBldglWithdrawals vo, String createContent) throws DaoException {
