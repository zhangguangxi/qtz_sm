...
/**
 * <p>Title:WtYccBldglWithdrawalsServiceImpl</p>
 * <p>Description:云仓储向便利店管理公司提现流水服务实现类</p>
 * <p>Copyright: Copyright (c) 2016</p>
  * <p>Company: 深圳擎天柱信息技术有限公司</p>
 * @author 徐运贤 - xuyunxian@126.com
 * @version v1.0 2016-05-11
 */
@Service("wtYccBldglWithdrawalsServiceImpl")
public class WtYccBldglWithdrawalsServiceImpl extends BaseServiceImpl<com.qtz.sm.wallet.vo.WtYccBldglWithdrawals,java.lang.Long> implements WtYccBldglWithdrawalsService  {
	/**初始化日志对象*/
	private static LogTool log = LogTool.getInstance(WtYccBldglWithdrawalsServiceImpl.class);
	/**注入云仓储向便利店管理公司提现流水DAO接口类*/
	@Resource(name="wtYccBldglWithdrawalsDaoImpl")
    private WtYccBldglWithdrawalsDao dao;
	...
	*/
	@Override
	protected BizDao<com.qtz.sm.wallet.vo.WtYccBldglWithdrawals,java.lang.Long> getDao() {
		return dao;
	}
	/** 
	* 【取得】日志对象
	...
	*/
	...
	public void saveApplyWithdrawals(WtYccBldglWithdrawals vo) throws ServiceException {
		...
		try {
			...
			//从配置文件中读取云仓储公司结算周期的设置参数
			String period=BaseProperties.getBaseProperties("ycc.withdrawals.period");
			...
			wVo.setOwnerType(CompanyType.CloudStorage.value());
			...
			for (WtWallet w : walletList) {	
				if (w.getPayerType() == CompanyType.StoreManager.value()) {
	...
	public void saveAccepWithdrawals(WtYccBldglWithdrawals vo) throws ServiceException {
		// TODO Auto-generated method stub
		try {
			WtYccBldglWithdrawals withdrawals = dao.findVo(vo.getDmId(), null);
			if (null == withdrawals) {
			...
			//获取收款人已付钱包
			WtWallet wPaid=wtWalletService.getPaidByTissue(wWithdrawals.getPayerId(), CompanyType.StoreManager.value(), CompanyType.CloudStorage.value(),WalletItemEnum.PAID.getValue());
			//获取收款人未付钱包
			WtWallet uwPaid=wtWalletService.getPaidByTissue(wWithdrawals.getPayerId(), CompanyType.StoreManager.value(), CompanyType.CloudStorage.value(),WalletItemEnum.UPPAID.getValue());
	 ...
	 * @throws DaoException 
	 */
	public void addWtWithdrawalsOperation(WtYccBldglWithdrawals vo, String createContent) throws DaoException {
